...
if (!this.currentUser) {
  ...
  mounted() {
    ...
    console.log(this.columnDefs.find(element => element.name == "action"));
    console.log(this.columnDefs[1]);
  },
  ...
  methods: {
    ...
    async fetchProductLabels() {
      const json = await axios.get("/productLabels").then((response) => {
    ...
    async fetchProductOrigins() {
      const json = await axios.get("/productOrigins").then((response) => {
    ...
    async fetchProductTypes() {
      const json = await axios.get("/productTypes").then((response) => {
...
class TotalValueRenderer {
  ...
  init(params) {
   ...
   this.eventListener = () => window.location.href = "/addproduct?product=" + this.cellValue;
